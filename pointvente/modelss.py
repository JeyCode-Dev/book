...
class Flocation(models.Model):
    ...
    dernierecloture = models.CharField(db_column='DERNIERECLOTURE', max_length=15, blank=True, null=True)  # Field name made lowercase.
    ...
    cptetaxe = models.CharField(db_column='CPTETAXE', max_length=16, blank=True, null=True)  # Field name made lowercase.
    cptetaxev = models.CharField(db_column='CPTETAXEV', max_length=16, blank=True, null=True)  # Field name made lowercase.
    cptechf = models.CharField(db_column='CPTECHF', max_length=16, blank=True, null=True)  # Field name made lowercase.
...
class Fmvts(models.Model):
    ...
    codeuser = models.ForeignKey(User, on_delete=models.SET_NULL, null=True)
...
class DetailRecettes(models.Model):
    ...
    qte=models.DecimalField(max_digits=4, decimal_places=2, default=0)
    # pu=models.DecimalField(max_digits=4, decimal_places=2, default=0)

class StockRecettes(models.Model):
    detail = models.ForeignKey(DetailRecettes, on_delete=models.SET_NULL, null=True)
    qte=models.DecimalField(max_digits=4, decimal_places=2, default=0)
    categorie = models.CharField(max_length=1,null=True)
    location = models.ForeignKey(Flocation, on_delete=models.SET_NULL, null=True)
...
class Droit(models.Model):
    class Meta:
        permissions = (
            ...
            ('Bon_Commande_Externe', 'Onglet Bon de Commande Externe'),
            ...
            ('recettesrapport', 'Onglet Recettes Rapport'),
            ('recettestock', 'Onglet Recettes Entrée Stock'),
            ('recettestocksortie', 'Onglet Recettes Facturation'),
            ...
            # ('Megas', 'Operation Megas'),
            # ('Exchange', 'Operation Exchange'),
            # ('Consultations', 'Consultations Operations Full'),
            # ('Princ', 'Caisse Principale'),
            # ('Second', 'Caisse Secondaire'),
            # ('CreerProduits', 'Creation Produits'),
            # ('Affectation', 'Affectation'),
            # # ('Modification', 'Modification Opération'),
            # ('Transfere', 'Transfere Caisse Prinipale'),
            # ('Cloture', 'Cloture'),
            # ('Monitoringpro', 'Monitoring Produits'),
            # ('Monitoringcais', 'Monitoring Caisse'),
            # ('Monitoringguich', 'Monitoring Guichet'),
            # ('Monitoringfac', 'Monitoring Facturation'),
            # ('Monitoringcat', 'Monitoring Catégorie'),
            # ('Cessions', 'Cessions'),
            # ('Rapport', 'Rapport'),
        )
